...
class TestReadTcp(unittest.TestCase):
    ...
    def test_empty_proc(self):
        ...
        try:
            _mk_proc(root, [])
            t = LifetimeTracker(8388, 1080, 1888, proc_root=root)
            self.assertEqual(t._read_tcp_conns(), {})
        finally:
            shutil.rmtree(root, ignore_errors=True)
...
class TestSessionLogic(unittest.TestCase):
    ...
    def test_long_normal_session_not_flagged_as_cut(self):
        """
        A long, healthy session (well past cut_max_sec) that ends normally
        must NOT be classified as a TSPУ cut, even if it happens to close
        with a reset. Only the cut_min_sec..cut_max_sec window is TSPУ's
        signature; longer sessions ending in RST are just ordinary
        ...
        fired = []
        t.on_cut = lambda lt: fired.append(lt)
        t._session_start = time.time() - 600   # 10 minutes - way past cut_max_sec
        t._last_activity = time.time() - 6
        t._yt_active = False
        t._tick()
        self.assertEqual(fired, [])

    def test_session_at_cut_max_boundary_is_flagged(self):
        root = self._mk_root()
        t = LifetimeTracker(8388, 1080, 1888, proc_root=root,
                            cut_min_sec=30, cut_max_sec=60, require_reset=False)
        fired = []
        t.on_cut = lambda lt: fired.append(lt)
        t._session_start = time.time() - 58   # inside the 30-60s window
        t._last_activity = time.time() - 6
        t._yt_active = False
        t._tick()
        self.assertEqual(len(fired), 1)

    def test_reset_window_sec_is_actually_enforced(self):
        """A reset far outside reset_window_sec must not confirm the cut."""
        root = self._mk_root()
        t = LifetimeTracker(8388, 1080, 1888, proc_root=root,
                            cut_min_sec=30, cut_max_sec=60, require_reset=True,
                            reset_window_sec=10)
        fired = []
        t.on_cut = lambda lt: fired.append(lt)
        t._reset_ts.append(time.time() - 120)   # way too old to confirm
        t._session_start = time.time() - 40
        t._last_activity = time.time() - 6
        t._yt_active = False
        t._tick()
