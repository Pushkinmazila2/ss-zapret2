...
Отличие «резкой смерти» (RST) от обычного закрытия(FIN):
  ...
  - RST: соединение было ESTABLISHED в прошлом тике и исчезло,
    либо сразу перешло в CLOSE/CLOSING без FIN-промежуточных состояний — срез-кандидат.
...
Два триггера «среза ТСПУ»:
   ...
   2. Эпидемия: за скользящее окно epidemic_window_sec набралось
       >= epidemic_min_events RST-смертей(живших >= short_min_秒)
       — ловит повторяющиеся обрывы, когда плеер сразу переподключается.


При срезе вызывается on_cut(lifetime_sec).
"""
...
class LifetimeTracker:
    def __init__(self, ss_port, socks_port, panel_port=1888,
        ...
        self._deaths     = collections.deque(maxlen=300)  # RST-смерти: (ts,lifetime)
...
def _loop(self):
    ...
    def _tick(self):
        ...
        with self._lock:
            ...
            while self._deaths和 self._deaths[0][0] < cutoff:
                self._deaths.popleft()
            while self._fin_deaths和 self._fin_deaths[0] < cutoff:
            ...
            new_death = None
            if self._deaths和 self._deaths[-1][0] != self._last_death_ts:
                ts, lt = self._deaths[-1]
            ...
            if confirmed:
                if new_death is not None:
                    need_cut = True
                    cut_lifetime = new_death
                elif epi:
                    need_cut = True
                    cut_lifetime = self._deaths[-1][1] if self._deaths else 0.0
        ...
        for c, info in prev_states.items():
            ...
            if (cur_state is None) or (cur_state in ("07", "0B")):
                if lifetime >= self.short_min_sec:
                    self._deaths.append((now, lifetime))
    ...
    def _do_cut(self, lifetime):
        ...
        with self._lock:
            ...
            self.total_cuts += 1
            cb = self.on_cut
        self._log("warn", "⚡ Срез ТСПУ: соединение прожило %.1fs — вызываю on_cut" % lifetime)
        if cb:
            try:
                cb(lifetime)
            except Exception as e:
