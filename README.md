...
### Детектор ТСПУ и журнал срезов соединений

Панель включает детектор блокировки ТСПУ: он следит за исходящими HTTPS-соединениями
(`:443`) через `/proc/net/tcp{,6}` и отличает «резкую смерть» (RST — соединение исчезло
без FIN-процедуры) от нормального закрытия. При срезе вызывается ротация стратегий пула.

Два триггера среза:
- **Классический** — одиночная RST-смерть с временем жизни в диапазоне
  «Мин./Макс. lifetime среза» (по умолчанию 30–60 с);
- **Эпидемия** — ≥ N коротких RST-смертей (по умолчанию 4 за 60 с) — ловит повторяющиеся
  обрывы, когда плеер сразу переподключается.

Каждый срез пишется в **журнал срезов** (карточка «Журнал срезов ТСПУ» в панели) с полным
контекстом: соединение, слот/стратегия, счётчики трафика, данные reset-монитора и хвосты
логов панели, nfqws2 и ss-server. Записи — JSONL; доступны «Скачать» и «Очистить» в UI,
эндпоинты `GET /api/cuts`, `GET /api/cuts/export`, `POST /api/cuts/clear`.

Файл журнала: `CUT_LOG_PATH` из `.env`, иначе `/opt/zapret2/logs/cuts.log`, если
смонтирован каталог `./logs` (переживает рестарт контейнера), иначе `/run/zapret-pool/cuts.log`.
Ротация по размеру (2 МБ × 3 копии). Для более подробных reset-событий в ss-server логе
установите `SS_VERBOSE=1`.

Слот в записи среза определяется по conntrack mark. Если procfs conntrack недоступен
(частый случай в контейнере), используется утилита `conntrack` (пакет `conntrack-tools`);
если и это не помогло — в журнал попадает самый активный живой слот
(`slot_resolved: false`) и хвосты **всех** живых слотов (`nfqws2_all_slots`).
Хвост слота содержит и маркеры жизненного цикла (`START CMD/STARTED/EXIT/FAILED`),
так что видно даже слот без трафика. Дополнительные аргументы каждому nfqws2
задаются через `NFQWS2_EXTRA_ARGS` (например, `--debug=2`), путь procfs conntrack —
через `NF_CONNTRACK_PROC`.
